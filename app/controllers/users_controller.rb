...
before_filter :authenticate, :except => [:new, :edit , :create , :update , :destroy , :papers , :instruction]
    ...
    def papers
        ...
        @questions = @paper.questions.paginate(:page => params[:page] ,:per_page => 10)
              ...
              @hash[count]=Array.new
              #raise @hash[count].inspect
              @hash[count] << q
              #raise @hash[count].inspect
              answers.each do |ans|
              #raise ans.inspect
            ...
            @q_array=@hash.keys.in_groups_of(5)
            #raise @q_array.inspect
           @alpha=['A','B','C','D']
